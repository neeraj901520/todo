...
rendernewtask() {
  ...
  // Priority dropdown
   ...
   static int32_t selected_priority = 0;
  {
    lf_push_font(&s.smallfont);
    lf_text("Priority");
    lf_pop_font();
    ...
    props.padding = 10.0f;
    props.margin_top = 0.0f; props.margin_bottom = 0.0f;props.margin_left=0.0f;
    props.text_color = LF_WHITE;
    props.border_width = 0.0f;
    props.corner_radius = 0.0f;
    lf_push_style_props(props);
    
    if(lf_button(items[selected_priority]) == LF_CLICKED){
        opened = true;
    }
